...
# --- Helper Function (Corrected for SVG format) ---
def rgb_to_hex(rgb):
    """Converts a NumPy RGB array to a valid SVG hex color string (e.g., #131e19)."""
    # Fix implemented: removed spaces between hex components
...
def vectorize_image_wu(image_path, n_colors=8, line_distance=5, hatch_angle=-45.0, stroke_thickness=1.0,
    ...
    for i, color_rgb in enumerate(color_palette):
        ...
        mask = np.all(mask_boolean_3d, axis=2).astype(np.uint8) * 255

        # CONTOUR EXTRACTION: Use RETR_TREE for hole hierarchy
    ...
    # --- GEOMETRIC PREPARATION for Diagonal Hatching ---
    center = (w // 2, h // 2)
    # M: Forward rotation (to make lines horizontal in rotated space)
    M = cv2.getRotationMatrix2D(center, hatch_angle, 1.0)
    # M_inv: Inverse rotation (to place lines back in final image space)
    M_inv = cv2.getRotationMatrix2D(center, -hatch_angle, 1.0)

    dwg = svgwrite.Drawing('output_vectorized.svg', size=(w, h), profile='tiny')
    print("Applying custom line fill (hatching) and saving to SVG...")

    # --- 5. Custom Line Fill (on Rotated Space) and 6. Save to Vector File (SVG) ---
    ...
    for data in color_data:
        ...
        # Only process OUTER contours (Parent index == -1)
        outer_contours_indices = [
            idx for idx, h_data in enumerate(hierarchy) if h_data[3] == -1
        ]
        ...
        for idx in outer_contours_indices:
            ...
            # --- ROTATE CONTOUR ---
            rotated_outer_contour = cv2.transform(outer_contour, M)
            x, y, bw, bh = cv2.boundingRect(rotated_outer_contour)

            # --- LINE SCAN in Rotated Space ---
            for y_line in range(y, y + bh, line_distance):
                ...
                for x_pos in range(x, x + bw):
                    ...
                    # 1. Check if inside the ROTATED outer contour
                    is_inside_outer = cv2.pointPolygonTest(rotated_outer_contour, point_rotated, measureDist=False) >= 0

                    # 2. Check if outside ALL HOLES (using the original contours/hierarchy with the rotated point)
                    # NOTE: This part is conceptually tricky. For simplicity/performance in a real app,
                    # one would also rotate the hole contours once outside this loop.
    ...
    dwg.save()
    print("Processing complete. Results saved to 'output_vectorized.svg'")
